export const services = [
  {
    id: 1,
    iconClass: "icon-strategic-planning",
    title: "Forex Investment Scams",
    description:
      "Recover funds lost to unregulated brokers, fake trading platforms, or Ponzi schemes.",
  },
  {
    id: 2,
    iconClass: "icon-operational-efficiency",
    title: "Cryptocurrency Investment Scams",
    description:
      "Trace and recover stolen BTC, ETH, and other digital assets using advanced blockchain forensics.",
  },
  {
    id: 3,
    iconClass: "icon-market-research",
    title: "Binary Options Scam",
    description:
      "Investigate and dispute unauthorized wire transfers through coordinated technical and banking action.",
  },
  {
    id: 4,
    iconClass: "icon-change-management",
    title: "Stock Trading Scams",
    description:
      "Assist with disputing fraudulent credit/debit card transactions to reclaim lost funds.",
  },
];

export const sectionServices = [
  {
    id: 7,
    imgSrc: "/image/section-services-item/section-services-item-1.jpg",
    title: "Retirement Planning",
    description: `
        Develop and implement long-term plans to achieve business goals. This
        <br />
        includes market analysis, internal assessments, and strategic
        <br />
        development.
     `,
    benefits: [
      "Reliable Income for Retirement",
      "Effective Tax Savings Strategies",
      "Secure Your Financial Future",
      "Comprehensive Asset Protection Plan",
    ],
  },
  {
    id: 8,
    imgSrc: "/image/section-services-item/section-services-item-2.jpg",
    title: "Investment Advisory",
    description: `
        Receive expert guidance on building a diversified investment portfolio
        <br />
        that aligns with your financial goals, risk tolerance, and long-term
        <br />
        growth objectives for maximum returns.
      `,
    benefits: [
      "Strategic Portfolio Diversification",
      "Tailored Investment Recommendations",
      "Risk Assessment and Mitigation",
      "Long-Term Wealth Growth Strategy",
    ],
  },
  {
    id: 9,
    imgSrc: "/image/section-services-item/section-services-item-3.jpg",
    title: "Estate Planning",
    description: `
        Create a solid estate plan to protect your assets, ensuring a smooth
        <br />
        transition of wealth to future generations while minimizing taxes and
        <br />
        legal complications.
      `,
    benefits: [
      "Smooth Wealth Transfer Solutions",
      "Customized Asset Protection Plans",
      "Tax-Efficient Wealth Strategies",
      "Secure Future Generations’ Inheritance",
    ],
  },
  {
    id: 10,
    imgSrc: "/image/section-services-item/section-services-item-4.jpg",
    title: "Tax Optimization",
    description: `
        Maximize your savings with personalized strategies to reduce tax
        <br />
        liabilities and enhance financial efficiency, keeping you prepared for
        tax
        <br />
        season and unforeseen expenses.
      `,
    benefits: [
      "Strategic Tax Reduction Methods",
      "Enhanced Savings Through Deductions",
      "Tailored Tax-Efficient Investments",
      "Year-Round Tax Planning Support",
    ],
  },
];

export const servicesData = [
  {
    id: "chargeback-wire-recall",
    imgSrc: "/image/section/section-services-1.jpg",
    imgWidth: 585,
    imgHeight: 439,
    title: "Chargeback and Wire Recall Assistance",
    description:
      "Need to reverse a scam payment? Folco Capital assists with chargebacks and wire recalls, guiding you through evidence submission and direct bank engagement.",
    benefits: [
      "Expert evidence compilation for chargeback claims",
      "Direct bank engagement and negotiation",
      "Wire recall process guidance",
      "Increased success rate in payment reversals",
    ],
    linkText: "Read More",
    isActive: true,
  },
  {
    id: "bec-fund-recovery",
    imgSrc: "/image/section/section-services-2.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Business Email Compromise (BEC) Fund Recovery",
    description:
      "Lost money to a business email scam? Folco Capital helps trace fraudulent transfers and recover funds diverted through spoofed or hacked email communications.",
    benefits: [
      "Email forensics and investigation",
      "Transfer tracing and fund location",
      "Legal action coordination",
      "Recovery of diverted business funds",
    ],
    linkText: "Read More",
    isActive: false,
  },
  {
    id: "fake-brokerage-recovery",
    imgSrc: "/image/section/section-services-3.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Fake Brokerage and Trading App Recovery",
    description:
      "Scammed by a fake broker or trading app? Folco Capital investigates the platform, tracks your funds, and helps recover your money through legal and financial actions.",
    benefits: [
      "Platform investigation and verification",
      "Fund tracking and tracing",
      "Legal enforcement actions",
      "Recovery of trading losses",
    ],
    linkText: "Read More",
    isActive: false,
  },
  {
    id: "phishing-email-scam",
    imgSrc: "/image/section/section-services-4.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Phishing and Email Scam Recovery",
    description:
      "Lost money to a phishing or email scam? Folco Capital traces transactions, investigates fraudulent emails, and helps recover your stolen funds with expert support.",
    benefits: [
      "Email fraud investigation",
      "Transaction tracing",
      "Financial recovery support",
      "Prevention guidance",
    ],
    linkText: "Read More",
    isActive: false,
  },
  {
    id: "investment-scheme-recovery",
    imgSrc: "/image/section/section-services-1.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Investment Scheme Recovery",
    description:
      "Folco Capital helps victims recover funds lost in Ponzi schemes, fake trading platforms, and deceptive investment opportunities. Get forensic recovery support now.",
    benefits: [
      "Ponzi scheme investigation",
      "Forensic fund tracing",
      "Legal recovery actions",
      "Investment fraud recovery",
    ],
    linkText: "Read More",
    isActive: false,
  },
  {
    id: "romance-dating-scam",
    imgSrc: "/image/section/section-services-2.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Romance and Online Dating Scam Recovery",
    description:
      "Scammed by someone you met online? Folco Capital recovers funds lost to romance scams and fake crypto investments. We trace payments, compile evidence, and take action.",
    benefits: [
      "Romance scam investigation",
      "Payment tracing",
      "Evidence compilation",
      "Recovery of stolen funds",
    ],
    linkText: "Read More",
    isActive: false,
  },
];

export const caseStudiesData = [
  {
    id: "tab1",
    tabTitle: "Financial Services",
    imgSrc: "/image/section/section-services-1.jpg",
    imgWidth: 585,
    imgHeight: 439,
    title: "Retirement Planning",
    description:
      "The financial sector, including banking and insurance, leverages digital technology to optimize transaction processes, enhance data security, and improve customer experiences.",
    benefits: [
      "25% Higher Retention: Personalized solutions.",
      "40% Efficiency Boost: Streamlined operations.",
      "20% Revenue Growth: Data-driven results.",
    ],
    isActive: true,
  },
  {
    id: "tab2",
    tabTitle: "Healthcare",
    imgSrc: "/image/section/section-services-2.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Investment Advisory",
    description:
      "The financial sector, including banking and insurance, leverages digital technology to optimize transaction processes, enhance data security, and improve customer experiences.",
    benefits: [
      "25% Higher Retention: Personalized solutions.",
      "40% Efficiency Boost: Streamlined operations.",
      "20% Revenue Growth: Data-driven results.",
    ],
    isActive: false,
  },
  {
    id: "tab3",
    tabTitle: "Retail & E-commerce",
    imgSrc: "/image/section/section-services-1.jpg",
    imgWidth: 585,
    imgHeight: 439,
    title: "Estate Planning",
    description:
      "The financial sector, including banking and insurance, leverages digital technology to optimize transaction processes, enhance data security, and improve customer experiences.",
    benefits: [
      "25% Higher Retention: Personalized solutions.",
      "40% Efficiency Boost: Streamlined operations.",
      "20% Revenue Growth: Data-driven results.",
    ],
    isActive: false,
  },
  {
    id: "tab4",
    tabTitle: "Manufacturing",
    imgSrc: "/image/section/section-services-3.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Tax Optimization",
    description:
      "The financial sector, including banking and insurance, leverages digital technology to optimize transaction processes, enhance data security, and improve customer experiences.",
    benefits: [
      "25% Higher Retention: Personalized solutions.",
      "40% Efficiency Boost: Streamlined operations.",
      "20% Revenue Growth: Data-driven results.",
    ],
    isActive: false,
  },
  {
    id: "tab5",
    tabTitle: "Energy & Utilities",
    imgSrc: "/image/section/section-services-4.jpg",
    imgWidth: 878,
    imgHeight: 659,
    title: "Tax Optimization",
    description:
      "The financial sector, including banking and insurance, leverages digital technology to optimize transaction processes, enhance data security, and improve customer experiences.",
    benefits: [
      "25% Higher Retention: Personalized solutions.",
      "40% Efficiency Boost: Streamlined operations.",
      "20% Revenue Growth: Data-driven results.",
    ],
    isActive: false,
  },
];

export const servicesData2 = [
  {
    id: "tab-6",
    tabTitle: "IT Strategy & Planning",
    title: "IT Strategy & Planning",
    description: (
      <>
        Safeguard your business with comprehensive cybersecurity <br />
        solutions, including threat detection, risk assessment, and advanced
        <br />
        protection against cyberattacks.
      </>
    ),
    benefits: [
      "Threat Monitoring",
      "Incident Response",
      "Firewall Management",
      "Risk Assessment",
      "Data Encryption",
      "Endpoint Security Tools",
    ],
    imgSrc: "/image/section/img-section-services-h8-2.jpg",
    imgWidth: 960,
    imgHeight: 720,
    isActive: false,
  },
  {
    id: "tab-7",
    tabTitle: "Cybersecurity Solutions",
    title: "Cybersecurity Solutions",
    description: (
      <>
        Safeguard your business with comprehensive cybersecurity <br />
        solutions, including threat detection, risk assessment, and advanced
        <br />
        protection against cyberattacks.
      </>
    ),
    benefits: [
      "Threat Monitoring",
      "Incident Response",
      "Firewall Management",
      "Risk Assessment",
      "Data Encryption",
      "Endpoint Security Tools",
    ],
    imgSrc: "/image/section/img-section-services-h8-1.jpg",
    imgWidth: 640,
    imgHeight: 480,
    isActive: true,
  },
  {
    id: "tab-8",
    tabTitle: "Cloud Services & Migration",
    title: "Cloud Services & Migration",
    description: (
      <>
        Safeguard your business with comprehensive cybersecurity <br />
        solutions, including threat detection, risk assessment, and advanced
        <br />
        protection against cyberattacks.
      </>
    ),
    benefits: [
      "Threat Monitoring",
      "Incident Response",
      "Firewall Management",
      "Risk Assessment",
      "Data Encryption",
      "Endpoint Security Tools",
    ],
    imgSrc: "/image/section/img-section-services-h8-3.jpg",
    imgWidth: 960,
    imgHeight: 720,
    isActive: false,
  },
  {
    id: "tab-9",
    tabTitle: "Managed IT Support",
    title: "Managed IT Support",
    description: (
      <>
        Safeguard your business with comprehensive cybersecurity <br />
        solutions, including threat detection, risk assessment, and advanced
        <br />
        protection against cyberattacks.
      </>
    ),
    benefits: [
      "Threat Monitoring",
      "Incident Response",
      "Firewall Management",
      "Risk Assessment",
      "Data Encryption",
      "Endpoint Security Tools",
    ],
    imgSrc: "/image/section/img-section-services-h8-4.jpg",
    imgWidth: 960,
    imgHeight: 720,
    isActive: false,
  },
];

export const allServices = [
  ...services,
  ...sectionServices,
  ...servicesData,
  ...caseStudiesData,
  ...servicesData2,
];
